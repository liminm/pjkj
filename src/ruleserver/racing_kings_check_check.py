...
class Checkmate():
    ...
    def calc_movesboard(self, fig_moves, occupied_positions):
        ...
        for i in range(0,len(fig_moves)):
            for fig in fig_moves[i]:
                ...
                if(i == 3):
                    knight_move = (fig >> 17) + (fig >> 15) + (fig >> 10) + (fig >> 6) + (fig << 6) + (fig << 10) + (fig << 15) + (fig << 17)
                ...
                if(i == 2 or i == 0):
                    ...
                    for j in range(8):
                        ...
                        elif (pos_exp + 9 * j <= 63 and pos_exp + 9 * j >= 0) and (len(set(diag_minus_range) & set(left_border)) == 0):
                            diag_minus_range.append(pos_exp + 9 * j)
                            break
                        else:
                            break
                    ...
                    for j in range(8):
                        if (pos_exp - 9 * j not in right_border and (pos_exp - 9 * j <= 63 and pos_exp - 9 * j >= 0)):
                            diag_minus_range.append(pos_exp - 9 * j)
                        elif (pos_exp - 9 * j <= 63 and pos_exp - 9 * j >= 0) and (len(set(diag_minus_range) & set(right_border)) == 0):
                            diag_minus_range.append(pos_exp - 9 * j)
                            break
                        else:
                            break
                    ...
                    for j in range(8):
                        ...
                        elif (pos_exp + 7 * j <= 63 and pos_exp + 7 * j >= 0) and (len(set(diag_plus_range) & set(right_border)) == 0):
                            diag_plus_range.append(pos_exp + 7 * j)
                            break
                        else:
                            break
                    ...
                    for j in range(8):
                        if (pos_exp - 7 * j not in left_border and (pos_exp - 7 * j <= 63 and pos_exp - 7 * j >= 0)):
                            diag_plus_range.append(pos_exp - 7 * j)
                        elif (pos_exp - 7 * j <= 63 and pos_exp - 7 * j >= 0) and (len(set(diag_plus_range) & set(left_border)) == 0):
                            diag_plus_range.append(pos_exp - 7 * j)
                            break
                        else:
                            break
                    ...
                    diag_minus_fig_slider = sum([(2**column) for column in range(len(diag_minus_range)) if (255<< (8*column) & fig)])
                    diag_plus_fig_slider = sum([(2**column) for column in range(len(diag_plus_range)) if (255<< (8*column) & fig)])
                    ...
                    if(int(form_string.format(diag_minus_mask)[::-1], 2)  >= - 2 *  int(form_string.format(diag_minus_fig_slider)[::-1], 2)        ):
                        ...
                        if(2 *  int(form_string.format(diag_minus_fig_slider)[::-1], 2) > sum([2**num for num in range(len(diag_minus_range))]) ):
                            diag_minus_right_line_moves = diag_minus_mask
                        else:
                            diag_minus_right_line_moves =    int(form_string.format( int(form_string.format(diag_minus_mask)[::-1], 2) - 2 * int(form_string.format(diag_minus_fig_slider)[::-1], 2)  )[::-1], 2)
                    # TODO test if case is working
                    else:
                        diag_minus_right_line_moves =  int(form_string.format(([2 ** num for num in range(len(diag_minus_range))] - (2 *  int(form_string.format(diag_minus_fig_slider)[::-1], 2) - int(form_string.format(diag_minus_mask)[::-1], 2))) )[::-1], 2)
                    ...
                    if(int(form_string.format(diag_plus_mask)[::-1], 2)  >= - 2 *  int(form_string.format(diag_plus_fig_slider)[::-1], 2)        ):
                        ...
                        if(2 *  int(form_string.format(diag_plus_fig_slider)[::-1], 2) > sum([2**num for num in range(len(diag_plus_range))]) ):
                            diag_plus_right_line_moves = diag_plus_mask
                        else:
                            diag_plus_right_line_moves =    int(form_string.format( int(form_string.format(diag_plus_mask)[::-1], 2) - 2 * int(form_string.format(diag_plus_fig_slider)[::-1], 2)  )[::-1], 2)
                    # TODO test if case is working
                    else:
                        diag_plus_right_line_moves =  int(form_string.format(([2 ** num for num in range(len(diag_plus_range))] - (2 *  int(form_string.format(diag_plus_fig_slider)[::-1], 2) - int(form_string.format(diag_plus_mask)[::-1], 2))) )[::-1], 2)
                    ...
                    bishop_moves = sum( [2** ( pos_exp - (int(pos_exp / 8)*9) + 9*num) for num in range( len(diag_minus_range) ) if( ((int(form_string_minus.format(  (diag_minus_line_moves >> num))[::-1], 2)) >> (len(diag_minus_range)-1)   ) != 0 ) ])  | sum( [2** ( pos_exp - (int(pos_exp / 8)*7) + 7*num) for num in range(len(diag_plus_range)) if( ((int(form_string_plus.format(  (diag_plus_line_moves >> num))[::-1], 2)) >> (len(diag_plus_range)-1)) != 0 ) ])
                ...
                if(i == 4 or i == 0 ):
                    ...
                    hor_fig_slider = sum([(((255<< (8 * row)) & fig) >> (8 * row)) for row in range(8) if (255 << (8 * row) & fig)])
                    ...
                    hor_line_moves = (np.uint8(hor_mask - 2 * hor_fig_slider)) ^ int('{:08b}'.format( np.uint8(int('{:08b}'.format(hor_mask)[::-1], 2) - 2 * int('{:08b}'.format(hor_fig_slider)[::-1], 2))  )[::-1], 2)
                    vert_line_moves = (np.uint8(vert_mask - 2 * vert_fig_slider)) ^ int('{:08b}'.format( np.uint8(int('{:08b}'.format(vert_mask)[::-1], 2) - 2 * int('{:08b}'.format(vert_fig_slider)[::-1], 2))  )[::-1], 2)
    ...
    # TODO which color is which player?
...
if __name__ == '__main__':
    ...
    white_figs, black_figs = board.split_color_figs(129,33024,9252,6168,16962,3855,61680)
    wh_indiv_figs = board.find_indiv_figs(white_figs)
    bl_indiv_figs = board.find_indiv_figs(black_figs)

    occupied_positions = board.set_occupied_pos(129,33024,9252,6168,16962)

    # 1. Queen 2. King 3. Bishop 4. Knight 5. Rook 6. White 7. Black 8. Player
    result = board.checkmate(129,33024,9252,6168,16962,3855,61680,'playerA')
    print(result)
